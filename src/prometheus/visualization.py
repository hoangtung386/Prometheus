"""Optional matplotlib helpers for eyeballing a batch.

Imported lazily by the notebook only. ``prometheus.api`` must never import this module:
``tests/contracts/test_core_imports.py`` asserts that the public API stays free of the
...
from .domain import NUCLEUS_CLASS_NAMES, TISSUE_CLASS_NAMES, MultitaskBatch

__all__ = ["visualize_multitask_batch"]
...
def visualize_multitask_batch(
    ...
    *,
    ...
    import matplotlib.pyplot as plt  # noqa: PLC0415 - optional `viz` extra
    from matplotlib.patches import Rectangle  # noqa: PLC0415 - optional `viz` extra
    ...
    tissue_plot = axes[1].imshow(tissue, cmap="tab10", vmin=0, vmax=len(TISSUE_CLASS_NAMES) - 1)
    ...
    if len(centroids):
        axes[2].scatter(
            ...
            cmap="tab10",
            vmin=0,
            vmax=len(NUCLEUS_CLASS_NAMES) - 1,
            s=14,
            marker="x",
            linewidths=1,
        )
