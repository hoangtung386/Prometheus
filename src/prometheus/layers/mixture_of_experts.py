...
class MoE(nn.Module):
    def __init__(self,
        ...
        self.experts = default(
            experts,
            lambda: Experts(dim, num_experts=num_experts, hidden_dim=hidden_dim, activation=activation),
        )
        self.loss_coef = loss_coef

    def forward(self, inputs, **kwargs):
        _, _, d, e = *inputs.shape, self.num_experts
...
class HierarchicalMoE(nn.Module):
    def __init__(self,
        ...
        self.gate_inner = Top2Gating(
            dim, num_gates=num_experts_inner, outer_expert_dims=(num_experts_outer,), **gating_kwargs,
        )
    ...
    def forward(self, inputs, **kwargs):
        _, _, d, eo, ei = *inputs.shape, self.num_experts_outer, self.num_experts_inner
...
HeirarchicalMoE = HierarchicalMoE
