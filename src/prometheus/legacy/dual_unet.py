...
from ..models.experimental.transformer import EncoderTransformerStack
from ..utils import LayerNorm
from .base_unet import build_decoder, build_encoder, forward_decoder, forward_encoder
    ...
    def forward(self, x: torch.Tensor, skips: list[list[torch.Tensor]]) -> tuple[torch.Tensor, torch.Tensor]:
        for level in range(3):
            ...
            for j, layer in enumerate(self.levels[level]):
                x = layer(x, stage_skips[0] if j == 0 else None)
...
class DualUNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.use_tissue_context:
            # The bridge naturally produces /16 features while the nuclei
            # encoder produces /32 features. Align them before attention.
            t_context = torch.nn.functional.adaptive_avg_pool2d(t_context, n_bottleneck.shape[-2:])
            context_seq = t_context.flatten(2).transpose(1, 2)

        Hf, Wf = n_bottleneck.shape[-2:]
        n_seq = n_bottleneck.flatten(2).transpose(1, 2)
        n_seq, moe_loss = self.transformer(
            n_seq,
            context=context_seq,
            spatial_size=(Hf, Wf),
        )
        ...
        if t_mask.shape[-2:] != output_size:
            t_mask = torch.nn.functional.interpolate(t_mask, size=output_size, mode="bilinear", align_corners=False)
        if n_mask.shape[-2:] != output_size:
            n_mask = torch.nn.functional.interpolate(n_mask, size=output_size, mode="bilinear", align_corners=False)
